...
<%
String where = "where destroymethodid > 0 ";
...
int count = DestroymethodManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+" "+sord+", destroymethodid";

DestroymethodBean[] punit = DestroymethodManager.getInstance().loadByWhere(where+" "+order+" "+limit);
...
for(int i=0;i<punit.length;i++){
    ...
    ob.put("id",punit[i].getDestroymethodid());
