...
<script type="text/javascript">
    ...
    function addPatient(id){
            $.post("content/patientdialog.jsp",{ id: id },function(data){ 
    ...
    function sendOrder(id){
        if(id > 0){
            $.post("content/sendorderdialogcdc.jsp",{ id: id },function(data){ 
                //var name = jQuery('#analizelist').jqGrid('getCell',id,'name');
...
//                datastr: tbldata,
                ...
                caption: '<b><i class="fa fa-user"></i> <%=trs.t("ადამიანი")%></b> <div class="dropdown pull-right"><a id="dLabel" role="button" data-toggle="dropdown" class="btn btn-primary" data-target="#" href="javascript:void(0);" aria-expanded="false"> <%=trs.t("ნიმუშის ტიპის შეცვლა")%> <span class="caret"></span> </a><ul class="dropdown-menu multi-level" role="menu"><li class="inline-group"><a href="javascript:showRegistry(\'registrycdcenv\');"><%=trs.t("გარემო")%></a></li><li><a href="javascript:showRegistry(\'registrycdcother\');"><%=trs.t("სხვა")%></a></li><li><a href="javascript:showRegistry(\'registrycdchumang\');"><%=trs.t("ადამიანი ჯგუფური")%></a></li><li><a href="javascript:showRegistry(\'registrycdcenvg\');"><%=trs.t("გარემო ჯგუფური")%></a></li><li><a href="javascript:showRegistry(\'registrycdcotherg\');"><%=trs.t("სხვა ჯგუფური")%></a></li></ul></div>'
            })
            ...
            .navButtonAdd('#pagerreestr', {id: 'addorder', caption: '<i class="fa fa-list-alt"></i> <%=trs.t("შეკვეთა")%>', buttonicon: "none", onClickButton: function () { addOrder(jQuery('#reestr').jqGrid('getGridParam','selrow')); }, position: 'last'})
            .navButtonAdd('#pagerreestr', {id: 'sendorder', caption: '<i class="fa fa-send"></i> <%=trs.t("გაგზავნა")%>', buttonicon: "none", onClickButton: function () { sendOrder(jQuery('#reestr').jqGrid('getGridParam','selrow')); }, position: 'last'})
            ;
