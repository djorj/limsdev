...
<%
    ...
    String labs = "";
    InstitutionBean[] insts = InstitutionManager.getInstance().loadByWhere("where parentid is not null and institutionid in (select institutionid from labtestinstitution where institutionid > 0)");
    for(int i=0;i<insts.length;i++){
        labs += "<option value='"+insts[i].getInstitutionid()+"'>"+insts[i].getName()+"</option>";
    }
%>
<style>
.ui-jqgrid-sortable {
...
.clevel4 {
    background-color: #64A0D4 !important;
}
        ...
        <div>
            <div class="smart-form col-sm-12" style="padding: 0;">
                <div class="col-sm-12">
                    ...
                    <section class="col custwidth">
                        <label class="select">
                            <select class="select2" name='author' placeholder="<%=trs.t("თანამშრომელი")%>">
                                <option value=""><%=trs.t("თანამშრომელი")%></option>
                                <%=labpers%>
                            </select>
                        </label>
                    </section>
                    ...
                    <section class="col custwidth">
                        <label class="input"> <i class="icon-append fa fa-folder"></i>
                            <input name="contract" type="text" placeholder="<%=trs.t("ხელშეკრულება")%>">
                        </label>
                    </section>
                    <section class="col custwidth">
                        <label class="input"> <i class="icon-append fa fa-folder-open"></i>
                            <input name="act" type="text" placeholder="<%=trs.t("აქტი")%>">
                        </label>
                    </section>
                    <section class="col custwidth">
                        <label class="input"> <i class="icon-append fa fa-folder-open-o"></i>
                            <input name="appeal" type="text" placeholder="<%=trs.t("მომართვა")%>">
                        </label>
                    </section>
                    <section class="col custwidth">
                        <label class="select">
                            <select class="select2" name='institutionid' placeholder="<%=trs.t("ლაბორატორია")%>">
                                <option value=""><%=trs.t("ლაბორატორია")%></option>
                                <%=labs%>
                            </select>
                        </label>
                    </section>
...
<script type="text/javascript">
    ...
    $("#frm1filter").keypress(function(e) { 
        if(e.keyCode == 13) {
            var op = $(this).serialize();
            $("#answers2").jqGrid('setGridParam',{url:"content/getanswerslist.jsp?"+op}).trigger("reloadGrid");
        }
    });
    ...
    function confirm2(cellvalue, options, rowObject){
        var level1 = rowObject[10];
        var tp = "warning";
        if(Number(cellvalue) > 0){
            tp = "success";
        }
        var btn = '';
        <%if(user.getPersonelltypeid().intValue() == 1 || user.getPersonelltypeid().intValue() == 9 || user.getPersonelltypeid().intValue() == 12){%>
        btn = '<button class="btn btn-'+tp+'" id="btn_2_'+rowObject[14]+'" onclick="setconfirm2('+rowObject[14]+',2)"><i class="fa fa-check"></i> </button>';
        <%}%>
        if(level1 == 0)
            btn = "";
        return btn;
    ...
    function confirm3(cellvalue, options, rowObject){
        var level2 = rowObject[11];
        var tp = "warning";
        if(Number(cellvalue) > 0){
            tp = "success";
        }
        var btn = '';
        <%if(user.getPersonelltypeid().intValue() == 1 || user.getPersonelltypeid().intValue() == 9){%>
        btn = '<button class="btn btn-'+tp+'" id="btn_3_'+rowObject[14]+'" onclick="setconfirm2('+rowObject[14]+',3)"><i class="fa fa-check"></i> </button>';
        <%}%>
        if(level2 == 0)
            btn = "";
        return btn;
    ...
    function printAnswer(id1,id2,id3,id4){
        /*$.post("content/checkconfirm.jsp",{ cid: id },function(data){
            if(data.result == 0)    alertError('< %=trs.t("ჩანაწერი არჩეული არ არის")%>',data.error,'< %=trs.t("დახურვა")%>');
            else printReport('F-003-2016-G',id);
        },"json");*/
        //printReport('F-003-2016-G',id);
        printReport('finalanswer',id1+"_"+id2+"_"+id3+"_"+id4);
        //console.log(id);
    }
    
    function printAnswer1(id1,id2,id3,id4){
        /*$.post("content/checkconfirm.jsp",{ cid: id },function(data){
            if(data.result == 0)    alertError('< %=trs.t("ჩანაწერი არჩეული არ არის")%>',data.error,'< %=trs.t("დახურვა")%>');
            else printReport('F-003-2016-G',id);
        },"json");*/
        //printReport('F-003-2016-G',id);
        printReport('internalanswerungrouped',id1+"_"+id2+"_"+id3+"_"+id4);
        //console.log(id);
    }
    
    function printAnswer2(id1,id2,id3,id4,id5){
        /*$.post("content/checkconfirm.jsp",{ cid: id },function(data){
            if(data.result == 0)    alertError('< %=trs.t("ჩანაწერი არჩეული არ არის")%>',data.error,'< %=trs.t("დახურვა")%>');
            else printReport('F-003-2016-G',id);
        },"json");*/
        //printReport('F-003-2016-G',id);
        printReport('finalanswer1',id1+"_"+id2+"_"+id3+"_"+id4+"_"+id5);
        //console.log(id);
    }
    
    function setColors(val,id){
        var confirmcolors = ['clevel0','clevel1','clevel2','clevel3','clevel4'];
        var html = $.parseHTML(val);
        $.each(html, function(i,el){
          if(i == 1){
              var obj = jQuery.parseJSON($(el).val());
//console.log(id+" obj.level = "+obj.level);
              $(id).addClass(confirmcolors[obj.level]);
              $(id+" td:nth-child(1)").addClass(confirmcolors[obj.level]);
          }
        });
    }
    ...
    $(document).on("click", ".jqgroup", function (e) {
            var id = $(this).attr("id");    //jQuery('#answers2').jqGrid('getGridParam', 'selrow');
            if (id != null) {                     
                    var ids = id.split("_");
                    var level2 = ids[0]+"_"+(Number(ids[1])+1)+"_"+(Number(ids[2])+1);
                    var level3 = ids[0]+"_"+(Number(ids[1])+2)+"_"+(Number(ids[2])+2);
                    jQuery('#answers2').jqGrid('groupingToggle',id);
                    jQuery('#answers2').jqGrid('groupingToggle',level2);
                    jQuery('#answers2').jqGrid('groupingToggle',level3);
...
//#739e73
      ...
      jQuery("#answers2").jqGrid({
        ...
        grouping:true,
        groupingView : { 
            groupField: ['num','samplename','barcode'],
            groupOrder: ['asc','asc','asc'],
            groupColumnShow: [false,false,false],
            hideFirstGroupCol: true,
            groupCollapse: true,
            groupText: ["{0}","{0}","{0} {1} კვლევა"],
            groupDataSorted: true
        }, 
        colNames: [
            '<%=trs.t("დამკვეთი")%>','<%=trs.t("ნიმუში")%>','<%=trs.t("მამა")%>', '<%=trs.t("კვლევა")%>', '<%=trs.t("ბარკოდი")%>', '<%=trs.t("ნიმუშის ტიპი")%>', '<%=trs.t("ნიმუშის მიღება")%>', 
            '<%=trs.t("ტესტის დასრულება")%>', '<%=trs.t("სავარაუდო დასრულება")%>', 
            '<%=trs.t("შენიშვნა")%>', '<%=trs.t("I დონე")%>', '<%=trs.t("II დონე")%>', '<%=trs.t("III დონე")%>','<%=trs.t("პასუხი")%>','sampleitemreseachid'],
        colModel: [
                    {name: 'num', index: 'num', hidden: false},
                    {name: 'samplename', index: 'samplename', hidden: true},
                    ...
                    {width: 80, name: 'barcodec', index: 'barcodec', editable: true, align: "left",sortable:"true"},
                    ...
                    {width: 80, name: 'printans', index: 'printans', editable: true, align: "center",sortable:"false"},
                ...
                onSelectRow: function (rowId) {
                    //$("#answers2").jqGrid ('toggleSubGridRow', rowId);
                },
                loadComplete: function () {
                    var i, group, headerIdPrefix = this.id + "ghead_",groups = $(this).jqGrid("getGridParam", "groupingView").groups,l = groups.length;
                    for (i = 0; i < l; i++) {
                        group = groups[i];
                        if(group.idx == 0)
                            setColors(group.value,"#" + headerIdPrefix + group.idx + "_" + i);
                        /*$("#" + headerIdPrefix + group.idx + "_" + i).on("click",function(){ 
                            $(".answers2ghead_0").removeClass("csel");
                            $(".answers2ghead_0 td:nth-child(1)").removeClass("csel");
                            $("#" + headerIdPrefix + group.idx + "_" + i).addClass("csel");
                            $("#" + headerIdPrefix + group.idx + "_" + i+"  td:nth-child(1)").addClass("csel");
                        });*/
                    }
                }
            })
