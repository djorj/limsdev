...
long appealid = 0;
long contractid = 0;
try{ contractid = Long.parseLong(request.getParameter("contractid")); }catch(Exception e){}
...
String sql = " and contractid = "+contractid;
if(actid > 0)   sql += " and actid = "+actid;
if(appealid > 0)   sql += " and appealid = "+appealid;
...
if(contractid > 0){
    ...
    // BOX parent
        ...
        ContractsampletypeBean[] list = ContractsampletypeManager.getInstance().loadByWhere("WHERE deleted=false AND isbox=true AND parentid is null "+sql+" ORDER BY contractsampletypeid");
    ...
    //BOX child
            ...
            {
                spc = "<label class=\"select\"><select class=\"selspicies\" id=\"spc_"+chld[i2].getContractsampletypeid()+"\" data-itemid=\""+chld[i2].getContractsampletypeid()+"\">";
                spc += "<option value=''></option>";
                for(int j=0;j<sps.length;j++){
                    String sel = "";
                    if(chld[i2].getSpeciesid() != null && sps[j].getSpeciesid().intValue() == chld[i2].getSpeciesid().intValue())
                        sel = "selected";
                    spc += "<option value='"+sps[j].getSpeciesid()+"' "+sel+">"+sps[j].getName()+"</option>";
                }
                spc += "</select></label>";
    ...
    // Normal parent
        list = ContractsampletypeManager.getInstance().loadByWhere("WHERE deleted=false AND isbox=false AND parentid is null "+sql+" ORDER BY contractsampletypeid");
        for(int i=0; i<list.length;i++)
        {
            String spc = "<label class=\"select\"><select class=\"selspicies\" id=\"spc_"+list[i].getContractsampletypeid()+"\" data-itemid=\""+list[i].getContractsampletypeid()+"\">";
            spc += "<option value=''></option>";
            ...
            if (list[i].getQuantity()>1)
            {
                ...
                for (int n=0; n<item.length; n++)
                {
                    ...
                    //if(count1 == 0)  
act += "<button class=\"btn btn-sm btn-primary showcode\" type=\"button\" data-itemid=\""+item[n].getContractsampleitemid()+"\" data-original-title=\""+trs.t("ბეჭდვა")+"\"><i class=\"fa fa-print\"></i></button> ";
                    ...
                    act += "<button class=\"btn btn-sm btn-default sampletypeinfoitem\" type=\"button\" data-original-title=\""+trs.t("შენიშვნა")+"\" data-itemid=\""+item[n].getContractsampleitemid()+"\"><i class=\"fa fa-commenting-o\"></i></button>";


                    spc = "<label class=\"select\"><select class=\"selspiciesitem\" id=\"spc_"+item[n].getContractsampleitemid()+"\" data-itemid=\""+item[n].getContractsampleitemid()+"\">";
                    spc += "<option value=''></option>";
                    for(int j=0;j<sps.length;j++){
                        String sel = "";
                        if(item[n].getSpeciesid() != null && sps[j].getSpeciesid().intValue() == item[n].getSpeciesid().intValue())
                            sel = "selected";
                        spc += "<option value='"+sps[j].getSpeciesid()+"' "+sel+">"+sps[j].getName()+"</option>";
                    }
                    spc += "</select></label>";
    ...
    {
        ContractsampletypeBean[] list = ContractsampletypeManager.getInstance().loadByWhere("WHERE deleted<>true AND isbox=true AND parentid is null "+sql);
        for(int i=0; i<list.length;i++)
        { 
            String spc = "<label class=\"select\"><select class=\"selspicies\" id=\"spc_"+list[i].getContractsampletypeid()+"\" data-itemid=\""+list[i].getContractsampletypeid()+"\">";
            spc += "<option value=''></option>";
...
act += "<button class=\"btn btn-sm btn-primary showcode\" type=\"button\" data-itemid=\""+item[0].getContractsampleitemid()+"\" data-original-title=\""+trs.t("ბეჭდვა")+"\"><i class=\"fa fa-print\"></i></button>";
        ...
        list = ContractsampletypeManager.getInstance().loadByWhere("WHERE deleted<>true AND isbox=false "+sql+" ORDER BY contractsampletypeid");
        for(int i=0; i<list.length;i++)
        {
            String spc = "<label class=\"select\"><select class=\"selspicies\" id=\"spc_"+list[i].getContractsampletypeid()+"\" data-itemid=\""+list[i].getContractsampletypeid()+"\">";
            spc += "<option value=''></option>";
