...
<script type="text/javascript">
    ...
    function setParams(id){
        BootstrapDialog.show({
            title: '<%=trs.t("დამატებითი პარამეტრები")%>',
            message: function(dialog){
                var $message = $('<div id="EXTRAPARAMDIALOG"></div>');
                $message.load("content/getextraparam.jsp?oper=sampletype&sampletypeid="+id);
                return $message;
            },
    ...
    var pagefunction = function () {
        ...
        function run_jqgrid_function() {
            jQuery("#materialtype").jqGrid({
                url:'content/getmaterialtypelist.jsp',
                datatype: "json",
                ...
                autowidth: true,
                rowList: [20, 50, 100],
                pager: jQuery('#pagermaterialtype'),
                sortname: 'name',
                viewrecords: true,
                sortorder: "asc",
                editurl: "content/updmaterialtype.jsp",
                title: "<%=trs.t("საანალიზო მასალის ტიპები")%>"
            })
            .jqGrid('filterToolbar', { searchOnEnter: true, enableClear: true })
            .navGrid('#pagermaterialtype', {edit:<%=permissions[1]%>,add:<%=permissions[0]%>,del:<%=permissions[2]%>,view:false,search:false},
                        {dataheight: 200, width: 400, closeAfterEdit: true, reloadAfterSubmit: true, afterSubmit: checkStatus,recreateForm:true,afterShowForm : function (formid) {rbtn(formid,'<%=trs.t("შენახვა")%>','<%=trs.t("დახურვა")%>');}}, //  default settings for edit
                        {dataheight: 200, width: 400, closeAfterAdd: true, reloadAfterSubmit: true, afterSubmit: checkStatus,recreateForm:true,afterShowForm : function (formid) {rbtn(formid,'<%=trs.t("შენახვა")%>','<%=trs.t("დახურვა")%>');}}, //  default settings for add
            ...
            .navButtonAdd('#pagermaterialtype', {caption: '<i class="fa fa-file-excel-o"></i> <%=trs.t("ექსპორტი")%>', buttonicon: "none", onClickButton: function () { }, position: 'last'})
            .navButtonAdd('#pagermaterialtype', {caption: '<i class="fa fa-wrench"></i> <%=trs.t("პარამეტრები")%>', buttonicon: "none", onClickButton: function () { setParams(jQuery('#materialtype').jqGrid('getGridParam','selrow')) }, position: 'last'})
            ;
            ...
            $(window).on('resize.jqGrid', function () {
                jQuery("#materialtype").jqGrid('setGridWidth', $("#content").width());
            })
...
<div class="table-size1">
    <table id="materialtype"></table>
    <div id="pagermaterialtype"></div>
</div>
