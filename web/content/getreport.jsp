...
try{    
	...
	if(pers1.length > 0)
        ...
        if(rs.length > 0){
            ...
            if(inst.getParentid() != null && inst.getParentid().intValue() == 2) inst2 = "../img/checkbox_checked_32.jpg";
            else if(inst.getParentid() != null && inst.getParentid().intValue() == 6) inst6 = "../img/checkbox_checked_32.jpg";
        ...
        String anamnesis = "";
        String address = "";
        ExtravalBean[] params = ExtravalManager.getInstance().loadByWhere("where extraparamid in (select extraparamid from extraparam where sampletypeid = "+st.getSampletypeid()+" and name = 'ანამნეზი') and contractsampleid = "+parent.getContractsampleitemid());
        if(params.length > 0 && params[0].getTval() != null)
            anamnesis = params[0].getTval();
        params = ExtravalManager.getInstance().loadByWhere("where extraparamid in (select extraparamid from extraparam where sampletypeid = "+st.getSampletypeid()+" and name = 'მისამართი') and contractsampleid = "+parent.getContractsampleitemid());
        if(params.length > 0 && params[0].getTval() != null)
            address = params[0].getTval();
        ...
        if(cm.getActid() != null){
            ...
            stamp = "აქტი #"+num+". ";
            stamp += (act.getStamp() != null && act.getStamp().trim().length() > 0) ? "დალუქული":"დაულუქავი";
            if(act.getBringer() != null) bringer = act.getBringer();
        }
        else if(cm.getAppealid() != null){
            AppealBean act = AppealManager.getInstance().loadByPrimaryKey(cm.getAppealid());
            num = act.getNum();
            stamp = "მომართვა #"+num+". ";
            stamp += (act.getStamp() != null && act.getStamp().trim().length() > 0) ? "დალუქული":"დაულუქავი";
            ...
            stamp = "ხელშეკრულება #"+contract.getNum()+". ";
            stamp += (contract.getStamp() != null && contract.getStamp().trim().length() > 0) ? "დალუქული":"დაულუქავი";
        }
        String registrationnumber = parent.getBarcode();
        String regdate = dt.format(sm.getRegdate());
        String sampletype = st.getName();
        if(sm.getDerivatetypeid() != null){
            DerivatetypeBean der = DerivatetypeManager.getInstance().loadByPrimaryKey(sm.getDerivatetypeid());
            sampletype = der.getName();
        }

        if(cm.getSpeciesid() != null){
            SpeciesBean sp = SpeciesManager.getInstance().loadByPrimaryKey(cm.getSpeciesid());
            if(sp.getName() != null && sp.getName().trim().length() > 0)
                sampletype += " - "+sp.getName();
        }
    ...
    } else if(reportname.equalsIgnoreCase("finalanswer1")){
        ...
        if(appealid > 0){
            ...
            hasstamp = "მომართვა #"+appeal.getNum()+" "+((appeal.getStamp() != null && appeal.getStamp().trim().length() > 0) ? "დალუქული #"+appeal.getStamp():"დაულუქავი");
            ...
            sql += " and actid = "+actid;
            hasstamp = "აქტი #"+act.getNum()+" "+((act.getStamp() != null && act.getStamp().trim().length() > 0) ? "დალუქული #"+act.getStamp():"დაულუქავი");
            if(contragent.getContragenttypeid().intValue() < 3 )
                sampleowner = contragent.getFname()+" "+contragent.getLname()+" "+contragent.getIdn();
            else
                sampleowner = contragent.getName() + " "+ contragent.getIdn();
            sampleowner += " "+act.getBringer();
            Recipient = sampleowner;
        } else {
            sql += " and contractid = "+contractid;
            samplegetdate = dt.format(contract.getRegdate());
            hasstamp = "ხელშეკრულება #"+contract.getNum()+" "+((contract.getStamp() != null && contract.getStamp().trim().length() > 0) ? "დალუქული #"+contract.getStamp():"დაულუქავი");
            if(contragent.getContragenttypeid().intValue() == 1){
                Recipient = "";
                if(contract.getContractownerid() != null){
                    ContractownerBean owner = ContractownerManager.getInstance().loadByPrimaryKey(contract.getContractownerid());
                    Recipient = owner.getFname()+" "+owner.getLname()+" "+owner.getIdn();
                }
                sampleowner = contragent.getFname()+" "+contragent.getLname()+" "+contragent.getIdn();
                Recipient += " "+sampleowner;
            } else {
                ...
                sampleowner += " " +contragent.getContact();
                Recipient += " "+sampleowner;
            }
        }
        ...
        sql = "select r.labtestid from sampleitemreseach r, contractsampleitem i, contractsampletype t where "
                ...
                + "t.sampletypeid = "+sampletypeid+" and "
                + "i.parentid = "+barcode;
        ...
        String sampleamount = items.length+" ("+Num2Text(items.length)+")";
