...
<%
String where = "where speciesid > 0 ";
...
int count = SpeciesManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+" "+sord+", speciesid";

SpeciesBean[] species = SpeciesManager.getInstance().loadByWhere(where+" "+order+" "+limit);
...
for(int i=0;i<species.length;i++){
    ...
    ob.put("id",species[i].getSpeciesid());
    JSONArray cell = new JSONArray();
    cell.add(trs.t(species[i].getName()));   
    ob.put("cell",cell);
    rows.add(ob);
}
obj.put("rows",rows);
