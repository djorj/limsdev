    ...
    var pagefunction = function () {
        ...
        function run_jqgrid_function() {
            ...
            jQuery("#list<%=gridname%>").jqGrid({
                url: 'content/get<%=gridname%>list.jsp?lab=true',
                datatype: "json",
                colNames: ['<%=trs.t("ბარკოდი")%>', '<%=trs.t("ბარკოდი")%>', '<%=trs.t("ნიმუშის ტიპი")%>',  '<%=trs.t("სახეობა")%>', '<%=trs.t("კვლევა")%>', '<%=trs.t("თარიღი")%>', '<%=trs.t("დამკვეთი")%>', '<%=trs.t("საველე&nbsp;N")%>', '<%=trs.t("ადგილმდებარეობა")%>', '<%=trs.t("სტატუსი")%>', '<%=trs.t("მოქმედება")%>','statusid'],
                colModel: [
                    {name: 'actid', index: 'actid', hidden: true, align: "center", width: 120},
                    {name: 'barcode', index: 'barcode', editable: true, align: "center", width: 70},
                    {name: 'sampletype', index: 'sampletype', editable: true, align: "left", width: 200},
                    {name: 'speciesid', index: 'speciesid', editable: true, align: "left", width: 100},
                    {name: 'labtestid', index: 'labtestid', hidden: true, align: "left", width: 100},
                    {name: 'regdate', index: 'regdate', editable: true, align: "center", width: 70},
                    {name: 'contragent', index: 'contragent', editable: true, align: "left", width: 120, hidden: true},
                    ...
                    {name: 'act', index: 'act', editable: true, align: "center", width: 140},
...
<div class="col-md-12 btn-panel">
    <button class="btn btn-primary pull-left" onclick="printForm();"><i class="fa fa-print"></i> <%=trs.t("შიდა ფორმა")%></button>
...
<div class="modal fade" id="unbanmodal" tabindex="-1" role="dialog" aria-labelledby="iconsModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                ...
                <h4 class="modal-title"><%=trs.t("დაწუნების გაუქმება")%></h4>
            </div>
            <div class="modal-body">
                <div class="smart-form">
<form id="frmunbanmodal" action="api/setsampletype.jsp" method="post" accept-charset="UTF-8">               
<input name="oper" type="hidden" value="status">
<input name="status" type="hidden" value="1">
<input name="id" type="hidden">
                    <section class="col col-sm-12">
                        <label class="label"><%=trs.t("ნამდვილად გინდათ დაწუნების გაუქმება?")%></label>
            ...
            <footer class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal"><%=trs.t("გაუქმება")%></button>
                <button type="button" class="btn btn-success" onclick="saveForm('unbanmodal')"><%=trs.t("დაწუნების გაუქმება")%></button>
            </footer>
        </div>
    </div>
</div>
...
<input name="oper" type="hidden" value="test">
<input name="id" type="hidden">
                    <section class="col col-sm-12">
...
<input name="id" type="hidden">
                    ...
                    <section class="col col-sm-12 con4">
                    ...
                    <section class="col col-sm-4 con4">
                        <label class="label"> <%=trs.t("მაცივარი")%></label>
                        <label class="input"> <i class="icon-append fa fa-database"></i>
                            <input name="refregerator" type="text">
                        </label>
                    </section>
                    <section class="col col-sm-4 con4">
                        <label class="label"> <%=trs.t("თარო")%></label>
                        <label class="input"> <i class="icon-append fa fa-database"></i>
                            <input name="shelf" type="text">
                        </label>
                    </section>
                    <section class="col col-sm-4 con4">
                        <label class="label"> <%=trs.t("ყუთი")%></label>
                        <label class="input"> <i class="icon-append fa fa-database"></i>
                    ...
                    <section class="col col-sm-12 con5" style='display: none;'>
                        ...
                        <label class="input"> <i class="icon-append fa fa-home"></i>
                            <input name="researcher"  type="text">
                        </label>
                    </section>
                    <section class="col col-sm-12 con5" style='display: none;'>
                        <label class="label"> <%=trs.t("ტრანსპ. პასუხისმგებელი პირები")%></label>
                        <label class="input"> <i class="icon-append fa fa-users"></i>
                            <input class="tagsinput" data-role="tagsinput" name="personells"  type="text" style="display: none;">
                        </label>
                    </section>
</form>
                </div>
            ...
            <footer class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal"><%=trs.t("გაუქმება")%></button>
                <button type="button" class="btn btn-success" onclick="saveForm('sendmodal')"><%=trs.t("შენახვა")%></button>
            </footer>
        </div>
    </div>
</div>
...
function setAliq() {
    ...
    else
    {
        ...
        $("#makemodalform").load("api/load_aliquote_derivate_form.jsp?id="+RowId.join(','), function() {
...
function setSend() {
    ...
    else
    {
        ...
        departamentid2select("frmsendmodal",RowId);
        $("#frmsendmodal input[name=id]").val(RowId);
        $("#frmsendmodal input[name=senddate]").val(cd.getDate() + "/"+ ("0" + (cd.getMonth()+1)).slice(-2) + "/"+ cd.getFullYear());
        $("#frmsendmodal input[name=sendtime]").val(cd.getHours() + ":"+ cd.getMinutes());
        $("#frmsendmodal input[name=researcher]").val("");
        $("#frmsendmodal input[name=receiver]").val("");
        $("#frmsendmodal input[name=container]").val("");
        $("#frmsendmodal input[name=personells]").val("");
        $("#frmsendmodal input[name=senddate]").datepicker({
        format: 'dd/mm/yyyy'
        });
        $("#frmsendmodal input[name=sendtime]").clockpicker({
	donetext: 'Done',
        twelvehour: false,
        autoclose: true
        });
...
$(document).on("click", ".showunban", function () {
    $("#frmunbanmodal input[name=id]").val($(this).attr("data-itemid"));
